...
const Blog = () => {
  ...
  return (
    <div ref={containerRef} className="px-6 md:px-16 py-12 ">
      {/* Heading */}
      ...
      {/* Posts Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 bg-[#F9F0D3]">
        {posts.map((post, index) => (
          <div
            ...
            className="flex flex-col sm:flex-row bg-[#F9F0D3] rounded-xl shadow-md overflow-hidden transition-transform duration-300 hover:shadow-xl hover:scale-105 cursor-pointer group"
          >
            {/* Image */}
            <img
              src={post.image}
              alt={post.title}
              className="w-full sm:w-1/2 h-56 object-cover transition-transform duration-500 group-hover:scale-110"
            />
